...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match res{
        ...
        Err(err) => {
            if err.eq("Description cannot be empty") || err.eq("Description cannot be longer than 500 bytes") {
                Ticket::new(title.clone(), "Description not provided".to_string(), status.clone()).unwrap()
            }
            else {
                return Ticket::new(title.clone(), description.clone(), status.clone()).unwrap();
            }
        },
    }
}
